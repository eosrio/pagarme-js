 ...
 * @module validations
 **/
import { __, ifElse, keys, map, mapObjIndexed, pick, pipe, prop, } from 'ramda';
import validations from './validate';
const pickExistentValidations = pipe(keys, pick);
const validation = prop(__, validations);
const mapIfArray = func => ifElse(Array.isArray, map(func), func);
const validateInput = (value, name) => mapIfArray(validation(name))(value);
 ...
 *                   or invalid.
 **/
export const validate = pipe(pickExistentValidations(validations), applyValidations);
