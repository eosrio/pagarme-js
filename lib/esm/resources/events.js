...
const findAllInTransactions = curry((opts, body) => request.get(opts, routes.events.transaction(body.transactionId), {}));
const findOneInTransactions = curry((opts, body) => request.get(opts, routes.events.transactionDetails(body.id, body.transactionId), {}));
const findAllInSubscriptions = curry((opts, body) => request.get(opts, routes.events.subscription(body.subscriptionId), {}));
const findOneInSubscriptions = curry((opts, body) => request.get(opts, routes.events.subscriptionDetails(body.id, body.subscriptionId), {}));
/**
 ...
 * @param {Object} body The payload for the request.
*/
const findCustom = (opts, body) => request.get(opts, routes.events.base, body);
export default {
    find,
    findCustom,
};
