 ...
 **/
import qs from 'qs';
import { toString, replace, pipe, } from 'ramda';
...
const cleanNumber = pipe(toString, replace(/[^0-9]/g, ''));
const queryString = card => qs.stringify({
 ...
 *
 */
const sign = (opts, string) => calculateSignature(opts.body.api_key, string);
/**
 ...
 *
 */
const verify = (opts, string, expected) => verifySignature(opts.body.api_key, string, expected);
export default {
    encrypt,
    sign,
    verify,
