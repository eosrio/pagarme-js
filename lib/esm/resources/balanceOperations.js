 ...
 **/
import { T, both, cond, curry, dissoc, has, pick, when, complement, isEmpty, applySpec, prop, pipe, } from 'ramda';
import * as routes from '../routes';
import request from '../request';
const findOne = curry((opts, body) => request.get(opts, routes.balanceOperations.details(body.id), {}));
const findAll = curry((opts, body) => request.get(opts, routes.balanceOperations.base, body || {}));
const findRecipients = curry((opts, body) => request.get(opts, routes.balanceOperations.recipients.findAll(body.recipientId), dissoc('recipientId', body)));
const findRecipientsWithBalanceId = curry((opts, body) => request.get(opts, routes.balanceOperations.recipients.find(body.id, body.recipientId), {}));
const pickQueryProps = pick(['startDate', 'endDate']);
const renameKeys = when(complement(isEmpty), applySpec({
    end_date: prop('endDate'),
    start_date: prop('startDate'),
}));
const buildQuery = pipe(pickQueryProps, renameKeys);
const findRecipientsWithFormat = curry((opts, body) => {
    const { recipientId, format } = body;
    return request.get(opts, routes.balanceOperations.recipients.findWithFormat(recipientId, format), buildQuery(body));
});
/**
 ...
 *                             The page index.
 */
const all = (opts, pagination) => findAll(opts, pagination);
/**
 ...
 * @param body
 */
const days = (opts, body) => request.get(opts, routes.balanceOperations.days, body || {});
export default {
    find,
    all,
    days,
};
//# sourceMappingURL=balanceOperations.js.map