...
const findOne = curry((opts, body) => request.get(opts, routes.paymentLinks.details(body.id), {}));
const findAll = curry((opts, pagination) => request.get(opts, routes.paymentLinks.base, pagination || {}));
/**
 * `GET /payment_links`
 ...
 */
const create = (opts, body) => request.post(opts, routes.paymentLinks.base, body);
/**
 ...
 *                    the request or to an error.
 */
const cancel = (opts, body) => request.post(opts, routes.paymentLinks.cancel(body.id), body);
export default {
    find,
    all,
    create,
