...
import {keys, length, mergeAll, mergeRight} from 'ramda';
import qs from 'qs';
...
const mergeWithDefaultHeaders = (headers: any) => {
	...
	return mergeRight(headers, defaultHeaders)
}
 ...
 * @private
 */
function buildRequestParams(method: string, endpoint: string, options: any, data: any) {
	let query = ''
	let body = ''
	let params;
...
function handleError(response: any) {
	...
	return response.json()
		.then((body: any) => Promise.reject(
			new ApiError(mergeRight(body, {status: response.status})),
		))
}
